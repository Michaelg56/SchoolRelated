...
public class AIPlayer extends Player{
	...
	private boolean partialWin(TicTacToe f, Player n){
		...
		else if((f.getBoard()[2][0] == check && f.getBoard()[1][1] == check && f.getBoard()[0][2] == '_') ||
	...
	public TicTacToe chooseMove(TicTacToe f){
		...
		for(int i = 0; i < s; i++){
			...
			//else if(partialWin(tmp, this) && !partialWin(tmp, opponent)){outComes.add(1.0);}
			else if(partialWin(tmp, opponent)){
				outComes.add(-2.0);
			}
			else{
